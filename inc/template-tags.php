...
function __tm_footer_logo() {
	...
	$logo_url = esc_url( __tm_render_theme_url( $logo_url ) );
	$logo_id  = __tm_get_image_id_by_url( __tm_render_theme_url( $logo_url ) );
 ...
 * @return void
 */
function __tm_social_list( $context = '', $type = 'icon' ) {
	...
	echo __tm_get_social_list( $context, $type );
}
...
function __tm_sticky_label() {
	...
	$sticky_type = get_theme_mod(
		'blog_sticky_type',
		__tm_theme()->customizer->get_default( 'blog_sticky_type' )
	);

	$content     = '';
	$icon_format = apply_filters( '__tm_sticky_icon_format', '<i class="fa %1$s"></i>' );
	...
	switch ( $sticky_type ) {
		...
		case 'icon':
			$icon = get_theme_mod(
				'blog_sticky_icon',
				__tm_theme()->customizer->get_default( 'blog_sticky_icon' )
			);
		...
		case 'label':
			$label = get_theme_mod(
				'blog_sticky_label',
				__tm_theme()->customizer->get_default( 'blog_sticky_label' )
			);
			$content = __tm_render_icons( $label );
			break;
		...
		case 'both':
			...
			$content = sprintf( $icon_format, $icon ) . __tm_render_icons( $label );
			break;
	}
...
function __tm_header_logo() {
	$type = get_theme_mod( 'header_logo_type', __tm_theme()->customizer->get_default( 'header_logo_type' ) );
	$logo = __tm_get_site_title_by_type( $type );
	...
	printf( $format, $tag, esc_url( home_url( '/' ) ), $logo, esc_attr( $type ) );
}
...
function __tm_get_site_title_by_type( $type ) {
	...
	$retina_logo     = '';
	$logo_url        = __tm_render_theme_url( $logo_url );
	$retina_logo_url = get_theme_mod( 'retina_header_logo_url' );
	$retina_logo_url = __tm_render_theme_url( $retina_logo_url );
	$logo_id         = __tm_get_image_id_by_url( $logo_url );
...
function __tm_site_breadcrumbs() {
	...
	$breadcrumbs_front_visibillity = get_theme_mod( 'breadcrumbs_front_visibillity', __tm_theme()->customizer->get_default( 'breadcrumbs_front_visibillity' ) );
 ...
 * @return bool
 */
function __tm_is_top_panel_visible() {
	$message = get_theme_mod( 'top_panel_text', __tm_theme()->customizer->get_default( 'top_panel_text' ) );
	$search  = get_theme_mod( 'top_panel_search', __tm_theme()->customizer->get_default( 'top_panel_search' ) );
	$menu    = has_nav_menu( 'top' );

	$conditions = apply_filters( '__tm_top_panel_visibility_conditions', array( $message, $search, $menu ) );
	$is_visible = false;
 ...
 * @return void
 */
function __tm_ads( $location ) {
	$ads    = trim( get_theme_mod( 'ads_' . $location, __tm_theme()->customizer->get_default( 'ads_' . $location ) ) );
 ...
 */
function __tm_ads_home_before_loop() {
	__tm_ads( 'home_before_loop' );
}

/**
 * Display ads for before loop content.
 */
function __tm_ads_post_before_content() {
	__tm_ads( 'post_before_content' );
}

/**
 * Display ads for before comments.
 */
function __tm_ads_post_before_comments() {
	__tm_ads( 'post_before_comments' );
}
