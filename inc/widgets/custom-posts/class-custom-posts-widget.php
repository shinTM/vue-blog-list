...
if ( ! class_exists( '__Tm_Custom_Posts_Widget' ) ) {
	...
	class __Tm_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', '__tm' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', '__tm' );
			$this->widget_id          = apply_filters( '__tm_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( '__tm_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = __tm_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', '__tm' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format' => array(
							'label' => esc_html__( 'Post Format', '__tm' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', '__tm' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', '__tm' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', '__tm' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', '__tm' ),
				),
				'meta_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						...
						'more_button'   => esc_html__( 'More Button', '__tm' ),
					),
					'label' > esc_html__( 'Display post meta data', '__tm' ),
				),
				'button_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', '__tm' ),
					'label' => esc_html__( 'Post read more button label', '__tm' ),
				),
			);
	...
	add_action( 'widgets_init', '__tm_register_custom_posts_widget' );
	function __tm_register_custom_posts_widget() {
