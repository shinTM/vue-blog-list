...
if ( ! class_exists( '__Tm_Taxonomy_Tiles_Widget' ) ) {
	...
	class __Tm_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', '__tm' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', '__tm' );
			$this->widget_id          = '__tm-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				...
				'layout_type' => array(
					...
					'options' => array(
						'grid' => array(
							'label' => esc_html__( 'Grid', '__tm' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', '__tm' ),
						),
					),
					'label' => esc_html__( 'Choose Layout Type', '__tm' ),
				),
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', '__tm' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				echo apply_filters( '__tm_taxonomy_tiles_widget_before', '<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '">', $instance );
				...
				echo apply_filters( '__tm_taxonomy_tiles_widget_after', '</div>', $instance );
			}
