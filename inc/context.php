...
function __tm_utility() {
	return __tm_theme()->get_core()->modules['cherry-utility'];
}
...
function __tm_header_class( $classes = array() ) {
	...
	echo __tm_get_container_classes( $classes, 'header' );
}
 ...
 * @return  void
 */
function __tm_header_container_class( $classes = array() ) {
...
function __tm_footer_class( $classes = array() ) {
	...
	echo __tm_get_container_classes( $classes, 'footer' );
}
 ...
 * @return string
 */
function __tm_get_container_classes( $classes, $target = 'content' ) {
	switch ( $target ) {
		case 'header':
...
function __tm_posts_list_class( $classes = array(), $echo = true ) {
	...
	foreach ( $sidebars as $sidebar ) {
		if ( __tm_widget_area()->is_active_sidebar( $sidebar ) ) {
			$has_sidebars = true;
		}
	}
